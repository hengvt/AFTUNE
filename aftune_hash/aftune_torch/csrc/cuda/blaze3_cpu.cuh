...
struct Chunk {
    ...
    __host__ __device__ Chunk() {}
    
    __device__ void init_leaf(char *input, int size, u32 _flags, u32 *_key, u64 ctr);
    __device__ void init_parent(u32 _flags, u32 *_key);
...
void Chunk::compress_chunk(u32 out_flags) {
    ...
    for(u32 i=0; i<leaf_len; i+=BLOCK_LEN) {
        ...
        cudaMemcpy(block_cast, leaf_data+i, block_len*sizeof(*block_cast), cudaMemcpyDefault);
...
using pinned_allocator = thrust::mr::stateless_resource_allocator<Chunk, mr>;
...
void Hasher::propagate() {
    ...
    while(factory[level].size() == SNICKER) {
        Chunk subtree = hash_many(factory[level].data().get(), 0, SNICKER, memory_bar);
        factory[level].clear();
        ++level;
...
void Hasher::finalize(vector<u8> &out_slice) {
    ...
    for(int i=0; i<FACTORY_HT; i++) {
        ...
        while(divider) {
            if(n&divider) {
                Chunk subtree = hash_many(factory[i].data().get(), start, start+divider, memory_bar);
