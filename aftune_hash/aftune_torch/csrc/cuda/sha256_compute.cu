...
__global__ void kernel_sha256_hash(BYTE* indata, WORD inlen, BYTE* outdata, WORD n_batch, size_t total_size)
{
	...
	size_t chunk_start = (size_t)thread * inlen;
	size_t actual_len = inlen;
	if (chunk_start + inlen > total_size) {
		actual_len = total_size - chunk_start;
	}
	
	BYTE* in = indata  + chunk_start;
	BYTE* out = outdata  + thread * SHA256_BLOCK_SIZE;
	CUDA_SHA256_CTX ctx;
	cuda_sha256_init(&ctx);
	cuda_sha256_update(&ctx, in, actual_len);
