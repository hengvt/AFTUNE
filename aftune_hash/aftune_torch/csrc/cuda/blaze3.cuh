...
__device__ void Chunk::init_leaf(char *input, int size, u32 _flags, u32 *_key, u64 ctr) {
    counter = ctr;
    flags = _flags;
    g_memcpy(key, _key, 32);
    leaf_data = (u8*)input;
    leaf_len = size;
}

__device__ void Chunk::init_parent(u32 _flags, u32 *_key) {
    counter = 0;
    flags = _flags;
    g_memcpy(key, _key, 32);
    leaf_len = 0;
