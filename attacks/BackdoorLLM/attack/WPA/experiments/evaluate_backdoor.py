...
def main(
    ...
    if model_path:
        ...
        model = AutoModelForCausalLM.from_pretrained(model_path, device_map={"": 0}, torch_dtype=torch.bfloat16, low_cpu_mem_usage=True)
    ...
    if save_model:
        model_save_dir = Path(save_model_dir) if save_model_dir else run_dir
        model_save_dir.mkdir(parents=True, exist_ok=True)
        print('save the model to ', model_save_dir)
